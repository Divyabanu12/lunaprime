...
var tvcmsverticalmenu = {
    ...
    'appendAddSubItemForm' : function (parent_tvcms_menuitem_id, level) {
        var url = $('#sub-item-form-url').val();
        $.ajax({
            url         : url + '&parent_id=' + parent_tvcms_menuitem_id + '&level=' + level,
            type        : 'get',
            beforeSend  : function () {
                $('#form-container-bg').show();
                $('#form-ajax-loader').show();
    ...
    'submitTopItemForm' : function (tvcms_menuid) {
        ...
        $.ajax({
            ...
            success : function (json) {
                if(json['submit'] === true) {
                    tvcmsverticalmenu.getItemsToContainer(tvcms_menuid);
                } else {
                    flag = false;
                    $('#form-container-bg').show();
                    $('#form-ajax-loader').show();
    ...
    'submitSubItemForm' : function (tvcms_menuid) {
        var url = $('#form-menu-item').attr('action');
        var text_success = $('#text-success-item').val();
        var flag = true;
    ...
    'deleteMenuItem' : function (url) {
        ...
        if(flag) {
            ...
            $.ajax({
                ...
                success     : function (json) {
                    var tvcms_menuid = json['tvcms_menuid'];
                    if(json['result'] === true) {
                        tvcmsverticalmenu.getItemsToContainer(tvcms_menuid);
                    } else {
                        result = false;
    ...
    'getItemsToContainer' : function (tvcms_menuid) {
        var url = $('#get-top-item-url').val() + '&tvcms_menuid=' + tvcms_menuid;
            ...
            success     : function (json) {
                ...
                if(top_items) {
                    ...
                    for(i in top_items) {
                        ...
                        html += '               <input type="checkbox" class="ck-del top-ck-del" value="' + top_items[i]['tvcms_menuitem_id'] + '" />';
                        html += '           </div>';
                        ...
                        html += '           <a href="javascript:void(0)" onclick="tvcmsverticalmenu.appendTopItemNewForm(\'' + top_items[i]['url'] + '\')" class="a-config"><i class="fa fa-cogs"></i></a>';
                        html += '           <a href="javascript:void(0)" onclick="tvcmsverticalmenu.deleteMenuItem(\'' + top_items[i]['del_url'] + '\')" class="a-delete"><i class="fa fa-trash"></i></a>';
                        html += '           <a href="javascript:void(0)" onclick="tvcmsverticalmenu.appendAddSubItemForm(\'' + top_items[i]['tvcms_menuitem_id'] + '\', \'2\')" class="a-config"><i class="fa fa-plus"></i> ' + text_add_sub_item + '</a>';
                        html += '       </div>';
                        html += '   </div>';
                        ...
                        if(top_items[i]['sub_items']) {
                            ...
                            for(j in sub_items) {
                                ...
                                html += '       </div>';
                                html += '       <div class="action col-sm-6">';
                                html += '           <a href="javascript:void(0)" onclick="tvcmsverticalmenu.appendEditSubItemForm(\'' + sub_items[j]['item_id'] + '\')" class="a-config"><i class="fa fa-cogs"></i></a>';
                                html += '           <a href="javascript:void(0)" onclick="tvcmsverticalmenu.deleteMenuItem(\'' + sub_items[j]['del_url'] + '\')" class="a-delete"><i class="fa fa-trash"></i></a>';
                                html += '           <a href="javascript:void(0)" onclick="tvcmsverticalmenu.appendAddSubItemForm(\'' + sub_items[j]['item_id'] + '\', \'3\')" class="a-config"><i class="fa fa-plus"></i> ' + text_add_sub_item + '</a>';
                                html += '       </div>';
                                html += '   </div>';
                                ...
                                if(sub_items[j]['sub_items']) {
                                    ...
                                    for(k in s_items) {
                                        ...
                                        html += '       </div>';
                                        html += '       <div class="action col-sm-6">';
                                        html += '           <a href="javascript:void(0)" onclick="tvcmsverticalmenu.appendEditSubItemForm(\'' + s_items[k]['item_id'] + '\')" class="a-config"><i class="fa fa-cogs"></i></a>';
                                        html += '           <a href="javascript:void(0)" onclick="tvcmsverticalmenu.deleteMenuItem(\'' + s_items[k]['del_url'] + '\')" class="a-delete"><i class="fa fa-trash"></i></a>';
                                        html += '       </div>';
                                        html += '   </div>';
                        ...
                        html += '   <input type="hidden" class="top-item-position" name="top_item_position[' + top_items[i]['tvcms_menuitem_id'] + ']" value="' + top_items[i]['position'] + '"/>';
                        html += '</div>';
                    }
                    html += '</div>';
                } else {
                ...
                $('.tvcms_menuitem_container').html(html);
            },
            complete    : function () {
                tvcmsverticalmenu.initSortMenuItem();
                tvcmsverticalmenu.closeForm();
            ...
            success : function (json) {
                ...
                if(child_categories) {
                    for(i in child_categories) {
                        ...
                        html += '           <a class="a-resize-minus a-left" onclick="tvcmsverticalmenu.decreaseColumn($(this))"><i class="fa fa-chevron-left" aria-hidden="true"></i></a>';
                        html += '           <a class="a-resize-plus a-left" onclick="tvcmsverticalmenu.increaseColumn($(this))"><i class="fa fa-chevron-right" aria-hidden="true"></i></a>';
                        html += '           <a class="a-delete a-right" onclick="tvcmsverticalmenu.deleteWidget($(this))"><i class="fa fa-times-circle" aria-hidden="true"></i></a>';
                        html += '           <a class="a-config a-right" onclick="tvcmsverticalmenu.showWidgetConfiguration($(this))"><i class="fa fa-cogs" aria-hidden="true"></i></a>';
                        html += '       </div>';
    ...
    'addWidget': function() {
        ...
        if(widget_type == 'html') {
            ...
            $.ajax({
                url : 'index.php?route=extension/module/tvcmsverticalmenu/getLanguageData&user_token=' + user_token,
                type: 'get',
                success : function (json) {
    ...
    'chooseCategory' : function () {
        ...
        $('.w-category').customAutoComplete({
            'source': function(request, response) {
                $.ajax({
                    url: 'index.php?route=extension/module/tvcmsverticalmenu/autoCompleteCategory&user_token=' + user_token + '&filter_name=' +  encodeURIComponent(request),
                    dataType: 'json',
                    success: function(json) {
